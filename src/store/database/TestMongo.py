...
class TestMongoDb(unittest.TestCase):
    ...
    def test_upsert(self):
        test_data = [{'test': 'test'}]
        with patch(self.mongo_client_namespace) as mongo_mock:
            self.db.database['stop_and_search'] = unittest.mock.Mock()
            self.db.upsert_many(test_data)

            test_item = test_data[0]
            update = pymongo.UpdateOne(test_item, {'$set': test_item}, upsert=True)
            self.db.database['stop_and_search'].bulk_write.assert_called_with(
                [update]
            )
    ...
    def test_upsert_with_nothing(self):
        ...
        with patch(self.mongo_client_namespace) as mongo_mock:
            ...
            self.db.database['stop_and_search'].bulk_write.assert_not_called()